...
import { View, Text, TextInput, TouchableOpacity, FlatList, Modal, Alert, StyleSheet, Platform } from "react-native";
...
const CustomerFormModal = ({ visible, onClose, onSave, initialData = null }) => {
    ...
    const handleSave = () => {
        ...
        const customerData = {
            // --- DÜZELTME 4: 'id' alanı SADECE düzenleme modunda (initialData varken) gönderilir. ---
            ...(initialData?.id && { id: initialData.id }), 
            // id: initialData?.id || Date.now().toString(), // BU SATIR DEĞİŞTİRİLDİ
        ...
        // onSave fonksiyonu artık 'await' ile bekleneceği için
        // 'onClose()' buradan kaldırıldı ve 'handleSaveCustomer' fonksiyonuna taşındı.
        onSave(customerData);
        // onClose(); // BURADAN KALDIRILDI
    };
...
const CustomerListItem = ({ item, onEdit, onDelete, orderCount }) => (
  <View style={customerListStyles.itemContainer}>
    ...
    <View style={customerListStyles.infoRow}>
        <View style={customerListStyles.infoItem}>
            <Ionicons name="call" size={14} color={Colors.iosBlue} />
            <Text style={customerListStyles.infoText}>{item.phone || "-"}</Text>
        </View>
        <View style={customerListStyles.infoItem}>
            <Ionicons name="mail" size={14} color={Colors.iosBlue} />
            <Text style={customerListStyles.infoText} numberOfLines={1} ellipsizeMode="tail">{item.email || "-"}</Text>
        </View>
    </View>
...
export default function CustomerScreen() {
  const { customers, addCustomer, updateCustomer, deleteCustomer, sales } = useContext(AppContext);
  ...
  // --- DÜZELTME 5: Fonksiyon 'async' yapıldı ve 'await' eklendi ---
  const handleSaveCustomer = async (customerData) => {
      let success = false;
      if (editingCustomer) {
          // 'customerData' zaten 'id' içeriyor (Düzeltme 4 sayesinde)
          success = await updateCustomer(customerData);
      } else {
          // 'customerData' 'id' içermiyor (Düzeltme 4 sayesinde)
          success = await addCustomer(customerData);
      }
      
      // --- DÜZELTME 6: Modal SADECE işlem başarılıysa kapatılır ---
      if (success) {
          setModalVisible(false);
          setEditingCustomer(null);
      } else {
          // Hata uyarısı zaten AppContext'te gösteriliyor.
