      ...
      <View style={styles.infoColumn}>
        ...
        <View style={styles.detailRow}>
            <Text style={styles.detailText}>
                <Text style={{fontWeight: '600'}}>{item.quantity} ad.</Text> x {Number(item.unitCost || 0).toFixed(2)} ₺
            </Text>
        </View>
        ...
        <View style={styles.detailRow}>
             ...
             <Text style={[styles.dateText, isOverdue && styles.overdueText]}>
                 {isDelivered 
                    ? `Teslim: ${new Date(item.deliveredDateISO).toLocaleDateString()}`
                    : (expectedDate ? `Beklenen: ${expectedDate.toLocaleDateString()}` : "Tarih Yok")
                 }
             </Text>
        </View>
      ...
      <View style={styles.actionsColumn}>
        {!isDelivered && (
          ...
          >
            ...
            <Text style={styles.deliverBtnText}>Teslim Al</Text>
          </TouchableOpacity>
        )}
        
        {isDelivered && (
             <View style={styles.deliveredBadge}>
                 <Text style={styles.deliveredBadgeText}>TAMAMLANDI</Text>
             </View>
        )}
